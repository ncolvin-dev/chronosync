...
class MeetingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Meeting::with('facility', 'assignments.volunteer.user')
            ->withoutTrashed();
        ...
        if ($request->filled('date_from')) {
            $query->where('date_scheduled', '>=', $request->date_from);
        }

        if ($request->filled('date_to')) {
            $query->where('date_scheduled', '<=', $request->date_to . ' 23:59:59');
        }
        ...
        $meetings = $query->orderBy('date_scheduled', 'desc')
            ->paginate(15);
        ...
        return view('meetings.index', compact('meetings', 'facilities'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'date_scheduled' => 'required|date|after:today',
        ...
        return DB::transaction(function () use ($validated) {
            $dateTime = Carbon::parse($validated['date_scheduled'] . ' ' . $validated['time_start']);
    ...
    public function show(Meeting $meeting)
    {
        ...
        return view('meetings.show', compact('meeting', 'assignments'));
    }
